...
if (length(inputs) < 2) {
    stop("usage: < dataset index > < replication no. >", call.=FALSE)
}
...
library(SPOTlight)
dir.create(paste0(results_path, "spotlight_optim/"), recursive=TRUE)
...
for (dataset_type in possible_dataset_types){
  ...
  saveRDS(spotlight_deconv, paste0(results_path, "spotlight_optim/", dataset, "_", dataset_type, "_spotlight.rds"))
}
