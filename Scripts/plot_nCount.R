...
for (i in 1:length(datasets)){
  ...
  if (i==2) {
    seuratObj <- subset(seuratObj, tech=="cell")
  } else if (i==3) {
    seuratObj <- subset(seuratObj, tech=="nucleus")
  }
  
  p[[i]] <- VlnPlot(seuratObj, features="nCount_RNA")
  ncount_list[[i]] <- seuratObj@meta.data[c("nCount_RNA", "nFeature_RNA")]
  png(paste0("~/thesis/plots/", datasets[i], "_nCount_RNA_bycelltype.png"))
  print(VlnPlot(seuratObj, features="nCount_RNA"))
  dev.off()
}

#saveRDS(p, "~/nCounts_plots.rds")
#saveRDS(ncount_list, "~/nCounts_list.rds")
