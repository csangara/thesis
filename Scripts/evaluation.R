...
for (dataset in datasets[2:length(datasets)]){
  for (repl in paste0('rep', 7)){
    if (repl %in% c('rep1', 'rep2', 'rep3')){ runs <- c("run1", "run2", "run3")
    } else { runs <- "" }
    for (run in runs){
      all_results <- list()
      result_path <- paste0("results/", dataset, "/", repl, "_", run, "/")
...
# mean
png("plots/mean_between_10reps.png", width=1200, height=500)
